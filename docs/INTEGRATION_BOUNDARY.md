...
RillML v1.5.5 owns the generic `rill-ml` library, runtime, protocol, model,
...
only. The current release plan, publisher, signed index builder, asset verifier,
and post-release smoke jobs must not build, publish, or require a PM adapter.

The PM-owned consumer selects its own published `rill-ml` dependency and carries
its own adapter source and lockfile. RillML v1.5.5 therefore does not claim
ownership of the PM adapter binary or its OpenWrt runtime behavior.
