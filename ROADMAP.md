...
> 项目定位：**RillML（简称 Rill）** 是面向原生应用与边缘设备的轻量自适应智能运行时（lightweight adaptive intelligence runtime）。
...
> 一个可直接嵌入 Rust 应用、能够持续学习、支持本地运行、状态可持久化、结果可诊断、适合真实产品使用的自适应智能运行时。
...
- （后续更新）发布索引 schema 保持 v2（`RELEASE_INDEX_SCHEMA_VERSION = 2`），并新增稳定 artifact id `rill-runtime-musl`。Linux 的 gnu 与 musl 资产通过 artifact ``id`` 区分（`rill-runtime` vs `rill-runtime-musl`），而非新增 schema 字段，从而不破坏 1.x Stable 的 release-index 契约。`linux-x86_64-musl`、`linux-aarch64-gnu`、`linux-aarch64-musl` 三个 Runtime 资产已纳入发布链（经 GitHub-Actions 宿主上的 Docker/QEMU 目标架构容器真实构建与执行产出）。
...
```text
宿主可以在不交出动作执行权的前提下，审计决策、延迟写回结果、恢复状态并离线回放
```
